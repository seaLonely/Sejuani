...
/**
 * 本地 AI 编码工具配置的持久化：镜像 aiConfig 的读-合并-写回模式，
 * 共用 ~/.sejuani/state.json 的 `coder` 键。
 *
...
export function setCoderConfig(patch: CoderConfigPatch): CoderConfig {
  ...
  const prev = state.coder && typeof state.coder === 'object' ? state.coder : {};
...
/** 状态文件路径（用于提示）。 */
export function coderStateFilePath(): string {
  return STATE_FILE;
}
